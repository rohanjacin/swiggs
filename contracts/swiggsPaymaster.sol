...
// Main Contract
contract SwiggsPaymaster is BasePaymaster, UniswapHelper, OracleHelper {

    using UserOperationLib for PackedUserOperation;

	// Paymaster config taken from account-abstraction/contracts
    ...
    event ConfigUpdated(PaymasterConfig paymasterConfig);
    ...
    constructor(
        ...
        PaymasterConfig memory _tokenPaymasterConfig,
    ...
    function setTokenPaymasterConfig(
        ...
        paymasterConfig = _tokenPaymasterConfig;
    ...
    returns (bytes memory context, uint256 validationResult) {unchecked {
            uint256 priceMarkup = paymasterConfig.priceMarkup;
            ...
            uint256 refundPostopCost = paymasterConfig.refundPostopCost;
        ...
        // note: as price is in native-asset-per-token and we want more tokens increasing it means dividing it by markup
            ...
            validationResult = _packValidationData(
                false,
                uint48(cachedPriceTimestamp + paymasterConfig.priceMaxAge),
                0
            );
        }
    }
    ...
    function _postOp(PostOpMode, bytes calldata context, uint256 actualGasCost, uint256 actualUserOpFeePerGas) internal override {
        ...
        // Refund tokens based on actual gas cost
            uint256 actualChargeNative = actualGasCost + paymasterConfig.refundPostopCost * actualUserOpFeePerGas;
    ...
    function refillEntryPointDeposit(uint256 _cachedPrice) private {
        ...
        if (
            currentEntryPointBalance < paymasterConfig.minEntryPointBalance
        ) {
